...
class LLMBase:
    ...
    - 所有 Agent 继承它，以保持一致性
    """
    def __init__(self, env_path: str, seed: Optional[int] = None):
    ...
    def call_structured_json(self, *, model: str, system_prompt: str, user_prompt: str,
                              json_schema: Dict[str, Any], temperature: float = 0.7) -> Dict[str, Any]:
        request_payload = {
            "component": "LLMBase.call_structured_json",
            "model": model,
            "temperature": temperature,
            "json_schema_name": json_schema.get("name") if isinstance(json_schema, dict) else None,
            "seed": self.seed,
